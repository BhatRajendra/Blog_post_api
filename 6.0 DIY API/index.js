...
app.get('/jokes/:id',(req,res)=>{
    ...
    //use line 20 or use == inside if condition
    ...
    //or use 
    //const joke=jokes.find((i)=>i.id===id); here i is each element in array
...
app.get('/filter',(req,res)=>{
    ...
    // type of joke as a query parameter.
    let allJokes=[];
    for(var i=0;i<jokes.length;i++){
        if(jokes[i].jokeType==type){
            allJokes.push(jokes[i]);
        }
    }
    res.json(allJokes);
});
...
app.post('/jokes',(req,res)=>{
    //type and text as a --body parameter.
    const id=jokes.length+1;
    const joke=req.body['text'];
    const type=req.body['type'];
    jokes.push({id:id,jokeText:joke,jokeType:type});
...
//write some error handlers..you can use flag ....if flag==0 send error object.
});
...
app.delete('/jokes/:id',(req,res)=>{
	...
	if(flag===1){
		...
		res.
			sendStatus(404);
			json({error:'mentioned id ${id} not found..!!!!'});
	}
